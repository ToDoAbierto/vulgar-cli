...
exports.generationWizard = function(options) {
  ...
  series([
    function(done) {
      var choices  = [{
        value: 'ng-routable',
        name: 'Routable Angular Component --> Front-End',
      }];
      ...
      inquirer.prompt(questions, function(results) {
        ...
        if(results.name.length > 0 ||
           results.path.length > 0) {
               ...
               env.lookup(function () {
                 env.run('vulgar:ng-route', { 'vulgarcli': true, 'name': results.name, 'path': results.path, 'dest': dest }, done);
               });
           }
