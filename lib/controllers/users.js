...
var authorize = exports.authorize = function(token, callback) {
	...
	if (token) {
		utils.updateGlobalVulgarJson({token: token}, function() {
			whoami();
		});
		...
		prompt.get({
			...
			function(err, result) {
				...
				utils.updateGlobalVulgarJson({token: result.token}, function() {
					whoami(callback);
				});
			});
...
exports.logout = function() {
	utils.updateGlobalVulgarJson({token:null}, function() {});
};
